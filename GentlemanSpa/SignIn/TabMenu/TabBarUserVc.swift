...
class TabBarUserVc: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if className == "Home" {
            ...
            if homeClass != nil {
                ...
                NotificationCenter.default.addObserver(self, selector: #selector(self.Cart_Count_Update), name: NSNotification.Name(rawValue: "Cart_Count_Update"), object: nil)
    ...
    @objc func Cart_Count_Update(_ notification: NSNotification) {
    ...
    func myCartAPI(){
        ...
        GetProductCartRequest.shared.GetCartItemsAPI(requestParams:[:], false) { [self] (arrayData,arrayService,message,isStatus,totalAmount) in
            if isStatus {
                if arrayData != nil{
                    var intCount = 0
                    var intCountProduct = 0

                    if arrayService?.allServicesArray.count ?? 0 > 0 {
                        intCount = arrayService?.allServicesArray.count ?? 0
                    }
                    if arrayData?.allCartProductArray.count ?? 0 > 0 {
                        intCountProduct = arrayData?.allCartProductArray.count ?? 0
                    }
                    intCount = intCount + intCountProduct
                    
                    lbeCount.isHidden = true
                    if intCount > 0 {
                        lbeCount.isHidden = false
                        lbeCount.text = String(intCount)
                    }
                }
                else{
                    lbeCount.isHidden = true
